...
1. Host function allocation
     ...
     * The `Cost` parameter can be 0 if developers not need the cost measuring.
     */
    WasmEdge_HostFunctionContext *HostFunc = WasmEdge_HostFunctionCreate(HostFType, Add, 0);

    /* If the host function instance not added into a import object context, it should be deleted. */
...
3. Specified import object
    ...
    Developers can create these import object contexts and register into the `Store` or `VM` contexts rather than adjust the settings in the `Configure` contexts.
...
The WasmEdge AOT (ahead-of-time) compiler compiles the WASM files for running in AOT mode which is much more faster than interpreter mode. Developers can compile the WASM files into the compiled-WASM files for the AOT mode running.
