...
Expect<uint32_t> WasiSockOpen::body(Runtime::Instance::MemoryInstance *MemInst, int32_t AddressFamily,
                        int32_t SockType, uint32_t RoFdPtr){
  /// Check memory instance from module.
  if (MemInst == nullptr) {
    return __WASI_ERRNO_FAULT;
  }
  ...
  auto Fd = socket(SysDomain, SysType, SysProtocol);
  if (unlikely(Fd < 0)) {
    return convertErrNo(errno);
  }
  ...
  *RoFd = static_cast<__wasi_fd_t>(Fd);

  return __WASI_ERRNO_SUCCESS;
}

Expect<uint32_t> WasiSockClose::body(Runtime::Instance::MemoryInstance *MemInst, int32_t Fd){
  /// Check memory instance from module.
...
Expect<uint32_t> WasiSockBind::body(Runtime::Instance::MemoryInstance *MemInst, int32_t Fd,
...
Expect<uint32_t> WasiSockListen::body(Runtime::Instance::MemoryInstance *MemInst, int32_t Fd, uint32_t Backlog){
  /// Check memory instance from module.
  if (MemInst == nullptr || Fd < 0) {
    return __WASI_ERRNO_FAULT;
  }
...
Expect<uint32_t> WasiSockAccept::body(Runtime::Instance::MemoryInstance *MemInst, int32_t Fd, uint32_t RoFdPtr){
  ...
  server_socket_addr.sin_family = AF_INET;
  server_socket_addr.sin_addr.s_addr = INADDR_ANY; 
  server_socket_addr.sin_port = htons(1234);
...
Expect<uint32_t> WasiSockConnect::body(Runtime::Instance::MemoryInstance *MemInst, int32_t Fd,
  ...
  client_socket_addr.sin_port = htons(stoi(port));
  if(inet_aton(address.c_str(), &client_socket_addr.sin_addr) < 0){
    return __WASI_ERRNO_FAULT;
  }
...
Expect<uint32_t> WasiSockRecv::body(Runtime::Instance::MemoryInstance *MemInst, int32_t Fd,
  ...
  /// Check for invalid address.
  char *const RiData =
      MemInst->getPointer<char *>(RiDataPtr, RiDataLen);
  if (unlikely(RiData == nullptr)) {
    return __WASI_ERRNO_FAULT;
  }
  ...
  *RoDataLen = return_size;

  return __WASI_ERRNO_SUCCESS;
}

Expect<uint32_t> WasiSockRecvFrom::body(Runtime::Instance::MemoryInstance *MemInst, int32_t Fd,
